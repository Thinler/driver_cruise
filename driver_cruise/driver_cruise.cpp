...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear)
{
	...
	{
		isFirstFrame = false;
		angleController.initial(10, 0.0000005, 12, targetAngleError);
	}
	...
	*cmdSteer = constrain(-1.0, 1.0, angleController.calculate(currentAngleError));
	...
	{
		speedController.initial(10, 0.000005, 0.8, targetSpeed);
	...
	if (currentSpeed < targetSpeed)  //当前速度低于目标，需要加速
	{
		...
		else if (_speed > 80 && abs(*cmdSteer) < 0.1)
		{
			//速度较慢且舵角较小时，限定油门
			*cmdAcc = 0.5;
		}
		else if (_speed > 10 && abs(*cmdSteer) < 0.1)
		{
			//速度较慢且舵角较小时，限定油门
			*cmdAcc = 0.8;
		}
